...
DEB_QINSTALLER="https://github.com/AlexandrDedckov/qinstaller/raw/master/qinstaller_1.1-1.deb"
DEB_QUANTRA="https://github.com/AlexandrDedckov/qinstaller/raw/master/python3-quantra_0.1.2-1_all.deb"
DEB_QUANTRALIB="https://github.com/AlexandrDedckov/qinstaller/raw/master/python3-quantralib_2.1.3-1_all.deb"
        ...
        r)
            ...
            if [ "$r" == "validator" ]; then
                ...
                if [ "$VERSION_CODENAME" == "xenial" ]; then
                    PKGS="$EOS_DEB_UBUNTU_16_04 $DEB_PKG $DEB_QINSTALLER"
                elif [ "$VERSION_CODENAME" == "bionic" ]; then
                    PKGS="$EOS_DEB_UBUNTU_18_04 $DEB_PKG $DEB_QINSTALLER"
                fi
            elif [ "$r" == "generator" ]; then
                TO_CHECK="quantra"
                if [ "$VERSION_CODENAME" == "bionic" ]; then
                    PKGS="$DEB_QUANTRA $DEB_QUANTRALIB $DEB_COLANDER $DEB_BASE58"
                elif [ "$VERSION_CODENAME" == "focal" ] || \
                     [ "$VERSION_CODENAME" == "groovy" ] || \
                     [ "$VERSION_CODENAME" == "hirsute" ]; then
                    PKGS="$DEB_QUANTRA $DEB_QUANTRALIB $DEB_COLANDER"
                fi
            elif [ "$r" == "user" ]; then
                TO_CHECK="quantrapy"
                if [ "$VERSION_CODENAME" == "bionic" ]; then
                    PKGS="$DEB_QUANTRALIB $DEB_COLANDER $DEB_BASE58"
                elif [ "$VERSION_CODENAME" == "focal" ] || \
                     [ "$VERSION_CODENAME" == "groovy" ] || \
                     [ "$VERSION_CODENAME" == "hirsute" ]; then
                    PKGS="$DEB_QUANTRALIB $DEB_COLANDER"
                fi
            else
                usage
            fi
